...
- (void)socket:(GCDAsyncSocket*)sock didReadData:(NSData*)data withTag:(long)tag
{
    ...
    if (tag == SOCKS_OPEN) {
        ...
        NSLog(@"TURNSocket: SOCKS_OPEN: ver(%o) mtd(%o)", version, method);
         ...
         */
        NSLog(@"TURNSocket: SOCKS_AUTH_USERPASS: %@", data);
        if (data.length == 2) {
            uint8_t* bytes = (uint8_t*)[data bytes];
            ...
            if (status == 0x00) {
                ...
                NSLog(@"TURNSocket: Invalid SOCKS username/password auth");
                [self.proxySocket disconnect];
                return;
            }
        } else {
            NSLog(@"TURNSocket: Invalid SOCKS username/password response length");
            [self.proxySocket disconnect];
            return;
        }
    } else {
