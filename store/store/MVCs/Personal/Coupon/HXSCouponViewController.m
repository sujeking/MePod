...
@interface HXSCouponViewController ()<UITableViewDelegate, UITableViewDataSource>

@property (nonatomic, weak) IBOutlet UITableView * tableView;

@property (nonatomic, strong) HXSCouponListRequest * request;
...
- (void)viewDidLoad {
    ...
    self.request = [[HXSCouponListRequest alloc] init];
    
    __weak typeof(self) weakSelf = self;
    [self.request getCouponsWithToken:[HXSUserAccount currentAccount].strToken
                            available:!self.isFromPersonalCenter
                                scope:self.couponScope
                             complete:^(HXSErrorCode code, NSString *message, NSArray *coupons) {
        if(code == kHXSNoError) {
