//
//  HXSDigitalMobileScrollableTabBar.m
//  store
//
...
static NSInteger kScrollTabbarSideButtonWidth = 15;
#define SCROLL_TABBAR_BAR_WIDTH  ((self.maxWidth - kScrollTabbarSideButtonWidth*2)/(self.numOfOnePage))

@interface HXSDigitalMobileScrollableTabBar () <UIScrollViewDelegate>
...
@property (nonatomic, assign) CGFloat    numOfOnePage;

@end

@implementation HXSDigitalMobileScrollableTabBar
...
- (void)initViews
{
    ...
    self.selectedImageView.backgroundColor = UIColorFromRGB(0x07A9FA);
    self.selectedImageView.hidden = YES;
    [self.tabScrollView addSubview:self.selectedImageView];
...
- (void)setItems:(NSArray *)items animated:(BOOL)animated width:(CGFloat)maxWidth number:(NSInteger)number
{
    if (0 < number) {
        self.numOfOnePage = number;
    }
    
    [self setItems:items animated:animated width:maxWidth];
}
...
- (void)insertTabWithItem:(NSDictionary *)itemDic atIndex:(NSUInteger)index animated:(BOOL)animated
{
    ...
    NSString *imageURLUnselectedStr = [itemDic objectForKey:kScrollBarImageUrlUnselected];
    NSString *imageURLSelectedStr   = [itemDic objectForKey:kScrollBarImageUrlSelected];
    NSNumber *countNum              = [itemDic objectForKey:kScrollBarCount];
    ...
    [button setImageForState:UIControlStateNormal
                     withURL:[NSURL URLWithString:imageURLUnselectedStr]];
    [button setImageForState:UIControlStateSelected
                     withURL:[NSURL URLWithString:imageURLSelectedStr]];
    
    [button setBackgroundColor:[UIColor clearColor]];
    if ((nil != countNum)
        && [countNum isKindOfClass:[NSNumber class]]
        && (0 >= [countNum integerValue])) {
